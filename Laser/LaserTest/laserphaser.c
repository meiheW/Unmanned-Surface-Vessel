...
int main(int argc, char *argv[])
{
	...
	struct ProjectData* pdata = (struct ProjectData*)malloc(udp_nums_per_cycle * 12 * 2 * 16);
	...
	//set-bubble-size
	setBubbleSize(start_angle, end_angle);
	while(1)
    {      
        ...
        for (int i = 0; i < 12; ++i) 
        {
            for (int j = 0; j < 32; ++j)
            {
                ...
                if (laser[i*32 + j].z < max_z && laser[i*32 + j].z > min_z && laser[i*32 + j].distance < 1000)
                { 
        ...
        if( udp_nums >= udp_nums_per_cycle ) //如果收满一周期的包
        {
			printf("udp_nums:%d\n", udp_nums_per_cycle);
			//***avoid obstacle	***//
			avoidObstacle(pdata, pindex, start_angle, end_angle);
